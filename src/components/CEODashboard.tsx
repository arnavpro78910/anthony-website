...
export const CEODashboard: React.FC<CEODashboardProps> = ({
  ...
  // VIP Client Accounts List (Synced with storage)
  const [vipUsers, setVipUsers] = useState<string[]>(() => {
    try {
      const stored = localStorage.getItem('company_ceo_vip_users_v1');
      if (stored) return JSON.parse(stored);
      // also include users with isVip
      return users.filter((u) => u.isVip || isUserVip(u)).map((u) => u.id);
    } catch {
      return [];
    }
  });
  ...
  const handleSaveGovernance = useCallback(async (updated: CompanyBranding) => {
    ...
    try {
      await syncBrandingToFirestore(updated);
      showNotification('Company Governance directives saved and synchronized live!', 'success');
    } catch {
      showNotification('Governance directives saved to local storage.', 'info');
    }
